...
//com vite para ler interações na view somente com algum evento, ai sim chama a funcao
$(document).on('click', 'button[data-user]', function () {
    const usuario = JSON.parse($(this).attr('data-user'));
    showUsuario(usuario);
});
...
function renderizarDadosEmpresa(usuario) {
    return `
        ...
        <span class='ml-3'> <strong> Empresa            : </strong> ${usuario.empresa?.nome_fantasia ? usuario.empresa.nome_fantasia.toUpperCase() : 'Não informado'}                                    </span> <br>
        <span class='ml-3'> <strong> Loja               : </strong> ${usuario.usuario_mercado?.loja?.nome ? usuario.usuario_mercado.loja.nome.toUpperCase() : 'Não informado'}                                </span> <br>
