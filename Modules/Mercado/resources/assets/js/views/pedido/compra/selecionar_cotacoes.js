...
var cotacoes = $('#dataView').data('cotacoes');
...
function formatDetails(cotacaoId) {
    ...
    cotacao.cot_fornecedores.forEach(cf => {
        html += `<tr class="fornecedor-row" data-id="${cf.id || ''}">
                            ...
                            <td>R$ ${gerais.centavosParaReais(cf.sub_total || 0)}</td>
                            <td>R$ ${gerais.centavosParaReais(cf.frete || 0)}</td>
                            <td>R$ ${gerais.centavosParaReais(cf.desconto || 0)}</td>
                            <td>R$ ${gerais.centavosParaReais(cf.total || 0)}</td>
                        </tr>`;
    });
...
function formatFornecedorItens(fornecedorId, cotacaoId) {
    ...
    fornecedor.cot_for_itens.forEach(item => {
        html += `<tr>
                            ...
                            <td>R$ ${gerais.centavosParaReais(item.preco_unitario || 0)}</td>
                            <td>R$ ${gerais.centavosParaReais((item.preco_unitario || 0) * (item.quantidade || 0))}</td>
                        </tr>`;
    });
...
$(document).ready(function () {
    const table = gerais.montaDatatable('tabela-cotacoes');
    ...
    $(document).on('click', 'td.details-control-sub', function () {
        ...
        if (!cotacaoId || isNaN(cotacaoId) || !fornecedorId || isNaN(fornecedorId)) {
            console.error('IDs inválidos ao expandir fornecedor:', {
                cotacaoId,
                fornecedorId
            });
        ...
        if ($tr.hasClass('shown')) {
            $('div.sub-slider', $tr.next()).slideUp(300, function () {
                ...
                $tr.find('.expand-icon-sub').removeClass('bi-chevron-down').addClass(
                    'bi-chevron-right');
            });
        } else {
            const rowHtml =
                `<tr><td colspan="6"><div class="sub-slider">${formatFornecedorItens(fornecedorId, cotacaoId)}</div></td></tr>`;
            $tr.after(rowHtml);
            $('div.sub-slider', $tr.next()).hide().slideDown(300);
            $tr.addClass('shown');
            $tr.find('.expand-icon-sub').removeClass('bi-chevron-right').addClass(
                'bi-chevron-down');
        }
    });
